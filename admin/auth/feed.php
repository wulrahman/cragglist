...
if($user->admin == 1) {
    ...
    if(isset($_POST['install'])) {
        ...
        $type_file = pathinfo($row->file_url, PATHINFO_EXTENSION);
        
        if(!space($type_file)) {
        
            $type_file = mime_content_types($row->file_url);

            $type_file = array_search($type_file, $setting['game_types']);
            
        }

        $array_file = array("swf", "unity3d", "dcr", "html", "htm");
        ...
        if($type_file == "html" || $type_file == "htm") {
        ...
        $type_image = pathinfo($row->thumb_url, PATHINFO_EXTENSION);
                ...
                <div>
                    ...
                    <nav class="pure-menu custom-restricted-width">
                        ...
                        <ul class="pure-menu-list">
                            ...
                            <li class="pure-menu-item"><a class="pure-menu-link" href="<?=$setting['games_url']?>/feed/all">All</a></li>

                            <?php
                    
                            $category_query = mysqli_query($setting['Lid'],"SELECT COUNT(*) AS `Rows`, `category` FROM `game_feed` GROUP BY `category` ORDER BY `category` LIMIT 10");
                            ...
                            while ($category = mysqli_fetch_object($category_query)) {
                                ...
                                echo '<li class="pure-menu-item"><a class="pure-menu-link" href="'.$setting["admin_url"].'/feed/'.$name.'">'.$category->category.'</a></li>';
                            ...
                            <li id="navbutton2" class="pure-menu-has-children pure-menu-allow-hover">
                                ...
                                <ul class="pure-menu-children">
                                    ...
                                    $category_query = mysqli_query($setting['Lid'],"SELECT COUNT(*) AS `Rows`, `category` FROM `game_feed` GROUP BY `category` ORDER BY `category` LIMIT 10, 100");
